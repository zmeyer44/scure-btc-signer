...
should('P2A output type', () => {
  ...
  deepStrictEqual(decoded, { type: 'p2a' });
...
should('Transaction input/output', () => {
  ...
  tx.updateInput(0, {
    ...
    index: 0,
    nonWitnessUtxo: nonWitnessUtxoHex,
  });
  deepStrictEqual(tx.inputs[0], {
...
should('return immutable outputs/inputs', () => {
  ...
  deepStrictEqual(tx.inputs[2].txid[0], 117);
  deepStrictEqual(i2.txid[0], 117);
  i2.txid[0] = 255;
  deepStrictEqual(tx.inputs[2].txid[0], 117);
...
should('unspendable output (gh-66)', () => {
  ...
  const prevTx = btc.Transaction.fromRaw(hex.decode(raw), { allowUnknownOutputs: true });
  const newTx = new btc.Transaction();

  // shouldn't crash!
  for (let index = 0; index < prevTx.outputs.length; index++)
